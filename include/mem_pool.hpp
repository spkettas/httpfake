...
MemPool<DataType>::MemPool(unsigned int PageSize, unsigned int MaxPage,int Mode)
{
	m_PageSize=PageSize;
	m_PageCnt=MaxPage*2;
	m_Mode=Mode;
	...
	//printf("Allocate %ubytes memory for user stat!total node=%d\n",m_PageSize*sizeof(DataType),m_PageSize);

}

template<typename DataType>
MemPool<DataType>::~MemPool()
{
...
bool MemPool<DataType>::append()
{
	...
	if(m_Mode == REUSABLE_MODE)
	{
		...
		_head[m_PageSize/2-1].addr = m_PageAddr[m_curpage+1]+m_PageSize/2-1;
	...
	//printf("Append %ubytes memory for user stat!add node=%d\n",m_PageSize/2*sizeof(DataType),m_PageSize/2);
	return true;
}

template<typename DataType>
DataType* MemPool<DataType>::GetNode()
{
	DataType* pmem=NULL;
	...
	{
		if( (m_cur == m_PageSize && m_curpage == 0) || (m_cur == m_PageSize/2 && m_curpage > 0) )
		{
			if(! append() )
				return NULL;
		}
...
template<typename DataType>
bool MemPool<DataType>::IsEmpty()
{
	return (m_freecnt==m_PageSize+(m_curpage)*m_PageSize/2);
}

template<typename DataType>
bool MemPool<DataType>::IsFull()
{
	return (m_freecnt==0);
}

template<typename DataType>
unsigned int MemPool<DataType>::UsedNodesCount()
{
	//TODO
...
template<typename DataType>
unsigned int MemPool<DataType>::NodesCount()
{
	//TODO
...
template<typename DataType>
unsigned int MemPool<DataType>::FreeNodesCount()
{
	//TODO
	return m_freecnt;
}
...
template<typename DataType>
void MemPool<DataType>::SetZero()
{
}

template<typename DataType>
void MemPool<DataType>::Reset()
{
	unsigned int i=0;
	if(m_Mode == REUSABLE_MODE)
	{
